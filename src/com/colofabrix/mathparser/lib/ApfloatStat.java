...
public class ApfloatStat {
    ...
    public static Apfloat erf_inverse( Apfloat z ) {
        ...
        for( int i = 0; i < ApfloatConsts.ERF_INVERSE_TAYLOR_MEMBER; i++ ) {
            sum = sum.add( coeff.get( i ).multiply( p ).precision( ApfloatConsts.DIVIDE_PRECISION ).divide( divider ) );
    ...
    public static Apfloat normalPDF( Apfloat x ) {
        Apfloat y = ApfloatConsts.ONE.precision( ApfloatConsts.DIVIDE_PRECISION ).divide(
                ApfloatMath.sqrt( ApfloatConsts.TWO.multiply( ApfloatConsts.PI ) ) );
    ...
    public static Apfloat Phi_inverse_ap( Apfloat x ) {
        ...
        Apfloat ui = x;
        if( ui.compareTo( ApfloatConsts.HALF ) != -1 )
            ui = ApfloatConsts.ONE.subtract( ui );
        ...
        // if(x < 0.5) szi = -szi;
        if( x.compareTo( ApfloatConsts.HALF ) == -1 )
            szi = szi.negate();
